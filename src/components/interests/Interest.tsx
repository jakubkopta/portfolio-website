...
const Interest = ({name, imagePath, description, position}: Props) => {
    ...
    return (
        ...
        >
            <img className="h-full object-cover opacity-30 group-hover:opacity-100 duration-300 rounded-xl"
                 src={imagePath} alt="img"/>
            <span
                className="opacity-80 group-hover:opacity-100 absolute font-bold text-6xl lg:text-8xl group-hover:text-5xl lg:-rotate-90 group-hover:rotate-0 group-hover:-top-10 group-hover:text-6xl ease-in duration-500 transform top-1/2 -translate-y-1/2">{name}</span>
