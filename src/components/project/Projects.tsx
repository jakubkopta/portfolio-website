...
import UsedLanguages from "./UsedLanguages.tsx";
    ...
    return (
        <div id="Projects" className="min-h-screen pb-10">
            ...
            <p className="text-center text-xl m-10 md:mt-10 md:m-28 md:mb-0">
                ...
                With a focus on Java and React, I dive into the world of software development to create innovative
                solutions and explore new possibilities.
                From building dynamic web applications with React to crafting robust backend systems with Java, I enjoy
            ...
            <div className="grid gap-12 m-10 md:m-28 md:mt-10 lg:grid-cols-3">
                {projectData.map((item, idx) => (
                    <Project key={idx} name={item.name} imagePath={item.imagePath} language={item.language}
                             icon={item.icon} githubName={item.githubName}/>
                ))}
            </div>
        </div>
    )
